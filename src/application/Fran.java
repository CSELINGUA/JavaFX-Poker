...
public class Fran extends Application {
	...
	// Variables pour la fonction de jetons
	...
	// Aide lorsque tu 'call'
	private int player1value = 0, player2value = 0;

	// Montant d'argent au milieu, dans le pot
	private int pot = 0;

	// Recevoir information de Evaluation.java
	String[] player1, player2;

	// Valeur du montant inscrit
	...
	public void start(Stage primaryStage) {
		...
		// Label2
		label2 = new Label("Quel est le nom du premier joueur?");
		label2.setLayoutX(30.0);
		label2.setLayoutY(5.0);

		// Label3
		label3 = new Label("Quel est le nom du premier joueur?");
		...
		back.setLayoutX(120);
		back.setLayoutY(130);
		networkPane.getChildren().addAll(dev, dev1, back);
		networkScene = new Scene(networkPane, 290, 200);
		net = new Button("Ordinateur");
		net.setLayoutX(58);
		net.setLayoutY(100);
		net.setOnAction(e -> {
	...
	private void finish() {
		...
		player2 = new String[] {};
		player2 = Evaluation.evaluate(map2.get("resultat2"), map2.get("dealerResultat"));
	...
	public String win(int un, int deux) {
		...
		if (String.valueOf(name).equals("player1")) {
			...
			map4.get("player2cash1").setText("Vous : " + map8.get("player2total") + "$");
			map4.get("player2cash2").setText("Adversaire : " + map8.get("player2total") + "$");
	...
	public void labels() {
		...
		labelText = new String[] { "Le montant maximum est " + limit, "Tour à " + player1Name, "Inscrire le montant",
				...
				"Adversaire : " + map8.get("player" + playerId + "total") + "$", "Vous n'avez pas d'argent!",
				"Vous n'avez pas assez d'argent!" };
		labelLayoutX = new int[] { 1150, 620, 1187, 1183, 1100, 1300, 1286, 11850, 310, 260, 500, 500, 800, 950, 800,
				950, 1150, 1150 };
	...
	public void clear() {
		...
		map4.get("player2numberTurn").setText("Tour : " + turnId);
		roundId++;
		map4.get("player1numberRound").setText("Ronde : " + roundId);
		map4.get("player2numberRound").setText("Ronde : " + roundId);
		bet = 10 * roundId;
		raiseAmount.setText(String.valueOf(bet));
		numberPanes();
		for (int i = 1; i <= totalPlayers; i++) {
